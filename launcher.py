...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Flux - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Flux",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Flux in case of issues",
                        action="store_true")
    parser.add_argument("--update-red",
                        help="Updates Flux (git)",
                        action="store_true")
...
def update_red():
    try:
        ...
        return
    if code == 0:
        print("\nFlux has been updated")
    else:
        print("\nFlux could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Flux has been restored to the last local commit.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Flux:")
        print("1. Update Flux + requirements (recommended)")
        print("2. Update Flux")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Flux (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            print("Are you sure? This will wipe ALL your Flux's installation "
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Flux. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Flux...")
                continue
            else:
                if not autorestart:
                    break

    print("Flux has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Flux without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Flux Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Flux has been "
        ...
        print("1. Run Flux /w autorestart in case of issues")
        print("2. Run Flux")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Flux needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Flux cannot work without the pip module. Please make sure to "
